...
import { Send, Trash2, Globe, Info, FileText, X, CheckCircle, Clock } from 'lucide-react';
...
interface Message {
  ...
  additionalInfo?: {
    fullText?: string;
    sources?: Source[];
    confidence?: number; // Opcional, por si lo añades en el futuro
    processingTime?: string; // Opcional
    internetUsed?: boolean; // Opcional
  };
}

// --- Componente de Estilos para el Scrollbar y Animación ---
const CustomStyles = () => (
  <style>{`
  ...
  <div className="flex items-center justify-center h-full">
    {[...Array(7)].map((_, i) => (
      <div
        key={i}
        className="wave-line"
        style={{ animationDuration: `${0.8 + i * 0.1}s`, animationDelay: `${i * 0.15}s` }}
      />
    ))}
  </div>
);
...
const AssistantAvatar = () => (
    <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden">
        <img src="https://i.imgur.com/2Y0b9c5.png" alt="Assistant Avatar" className="w-full h-full object-cover" />
    </div>
);
                ...
                <>
                    ...
                    {info.sources && info.sources.length > 0 && (
                         <div className="space-y-3 pt-4 mt-4 border-t border-gray-200">
                            <h4 className="font-semibold text-gray-800 mb-2">Fuentes:</h4>
                            <ul className="space-y-2">
                                {info.sources?.map((source: Source, index: number) => (
                                    <li key={index} className="p-2 border rounded-md bg-gray-50 text-xs">
                                        ...
                                        <p className="text-gray-600 mt-1 line-clamp-2">{source.page_content}</p>
                                    </li>
                                ))}
                            </ul>
                        </div>
...
const App: React.FC = () => {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] w-full bg-gray-100 flex overflow-hidden font-sans">
      <CustomStyles />
      <InfoPanel
      ...
      <main className={`flex-1 flex items-center p-4 transition-all duration-300 ease-in-out ${isInfoPanelOpen ? 'justify-start' : 'justify-center'}`}>
        <div className="w-full max-w-4xl h-full flex flex-col">
            ...
            <div className="flex-1 flex flex-col min-h-0 bg-gradient-to-br from-blue-400 via-cyan-400 to-indigo-500 rounded-2xl p-1 shadow-2xl">
              {/* Contenedor blanco interior que gestiona el layout del chat */}
              <div className="w-full h-full flex flex-col bg-white rounded-xl overflow-hidden">
                {/* Cabecera del Chat */}
                <div className="bg-white border-b border-gray-200 p-4">
                  <div className="flex items-center justify-between">
                    <h1 className="text-xl font-semibold text-gray-900">Asistente de Documentos</h1>
                ...
                <div className="bg-white border-t border-gray-200 p-4">
                  <div className="flex items-end space-x-3">
                    <div className="flex-1">
                      <textarea value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} onKeyPress={handleKeyPress} placeholder="Pregunta lo que sea sobre tus documentos..." className="w-full px-4 py-3 border border-gray-300 rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 h-20"/>
                    </div>
